...
namespace _24DH112473_MyStore.Controllers
{
    public class OrderController : Controller
    {
        ...
        public ActionResult Checkout()
        {
            ...
            //Ở đây tôi lấy địa chỉ của khách hàng và gán vào ShippingAddress của CheckoutVM
            ...
            var model = new CheckoutVM() //Tạo dữ liệu hiển thị cho CheckoutVM
            {
                ...
                // Compute total here to avoid relying on a non-persisted field
                TotalAmount = cart.Sum(item => item.Quantity * item.UnitPrice), //Tổng giá trị của các mặt hàng trong giỏ
                OrderDate = DateTime.Now, //Lấy thời điểm đặt hàng
                ShippingAddress = customer.CustomerAddress, //Lấy địa chỉ mặc định từ bảng Customer
                CustomerId = customer.CustomerID, // <-- FIX: use CheckoutVM.CustomerId (consistent property)
        ...
        public ActionResult Checkout(CheckoutVM model)
        {
            if (ModelState.IsValid)
            {
                ...
                // Compute total amount server-side to avoid tampering/mismatch
                var computedTotal = cart.Sum(i => i.Quantity * i.UnitPrice);
                ...
                var order = new Models.Order
                {
                    ...
                    // Map shipping address into EF model property
